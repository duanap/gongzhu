...
<template>
  <section class="table-seat" :class="{ active, current }">
    ...
    <div class="seat-copy">
      <strong>{{ displayName(player.name) }}<i v-if="player.isBot" class="seat-name-ai"> · AI</i></strong>
      <span>{{ label }} · {{ player.handCount || 0 }} 张</span>
      <small>
        <b v-if="player.passed" class="seat-pill passed">已传牌</b>
        <b v-if="player.aiControlled || player.takeoverFromName" class="seat-pill ai">托管</b>
        {{ statusText(player) }}
      </small>
    </div>
